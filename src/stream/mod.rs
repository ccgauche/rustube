...
impl Stream {
    ...
    async fn internal_download_to<P: AsRef<Path>>(
        ...
        let result = match self
            .download_full(
                ...
                &channel,
                0,
                self.content_length.load(Ordering::SeqCst) as usize,
            )
            .await
        {
            Ok(_) => {
                ...
                if matches!(e.status(), Some(reqwest::StatusCode::NOT_FOUND)) =>
            {
                ...
                Err(Error::Internal(
                    "try to download using sequenced download".into(),
                ))
    ...
    /* async fn download_full_seq(
        ...
        let res = self.get(&url, 10_000_000).await?;
    ...
    #[inline]
    async fn get(&self, url: &url::Url, length: usize) -> Result<reqwest::Response> {

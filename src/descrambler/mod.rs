...
fn apply_signature(streaming_data: &mut StreamingData, js: &str) -> crate::Result<()> {
        ...
        .chain(streaming_data.adaptive_formats.iter_mut())
    {
        ...
        let ratebypass = !url.query_pairs().any(|(key, _)| key == "ratebypass");
        if ratebypass {
            let initial_n = url
                .query_pairs()
                .find(|(x, _)| x == "n")
                .map(|(_, n)| n.as_ref().to_owned());
            let mut other_cipher: new_cipher::Cipher = new_cipher::Cipher::new(js);
            let new_n = other_cipher
                .calculate_n(initial_n.map(|x| x.chars().collect()).unwrap_or_default());
            let values = url
                .query_pairs()
            ...
            for (x, y) in values {
                if x == "n" {
                    ...
                    url.query_pairs_mut().append_pair(&x, &y);
                }
            }
            url.query_pairs_mut().append_pair("n", &new_n);
        }

        url.query_pairs_mut().append_pair("sig", s);
        url.query_pairs_mut()
            .append_pair("player_client", "android");
        out.push(url.to_string());
    }
    std::fs::write("url.txt", out.join("\n").as_str()).unwrap();
